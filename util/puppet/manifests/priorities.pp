...
postgresql::database { "priorities":
  owner => "vagrant",
}

exec { "load postgis":
  command => "/usr/bin/psql -f /usr/share/postgresql/9.1/contrib/postgis-1.5/postgis.sql -d priorities",
  user => "vagrant",
  require => Postgresql::Database['priorities']
}

exec { "load spatialrefs":
  command => "/usr/bin/psql -f /usr/share/postgresql/9.1/contrib/postgis-1.5/spatial_ref_sys.sql -d priorities",
  user => "vagrant",
  require => Postgresql::Database['priorities']
}
...
python::venv::isolate { "/usr/local/venv/priorities":
  subscribe => [Package['python-mapnik'], Package['build-essential']]
}

file { "settings_local.py":
  path => "/vagrant/priorities/settings_local.py",
